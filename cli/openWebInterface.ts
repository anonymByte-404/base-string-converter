...
export const openWebInterface = async (): Promise<void> => {
  try {
    ...
    const reactProcess: ChildProcess = spawnReactApp(scriptPath)
...
const getScriptPath = (): string => {
  ...
  const script: string = isWindows ? './startFrontend.bat' : './startFrontend.sh'
 ...
 */
const spawnReactApp = (scriptPath: string): ChildProcess => {
