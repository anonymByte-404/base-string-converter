  ...
  try {
    ...
    if (selectedBase === 'Exit the application') {
      ...
      if (baseMatch) {
        ...
        await universalBaseConverter(inquirer, main, typewriterEffect, fadeOutEffect, chalk, base)
      } else {
...
const handleStringConversion = async (): Promise<void> => {
  try {
    await stringConverter(inquirer, baseChoices, main, typewriterEffect, fadeOutEffect, chalk)
  } catch (error: unknown) {
